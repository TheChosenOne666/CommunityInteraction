package com.xiaolou.community.manager.cache;
...
public class CacheManager {
    ...
    public void putIfPresent(String hashKey, String key, Object value) {
        String compositeKey = buildCacheKey(hashKey, key);
        // 写操作不触发热点探测，直接检查是否已在本地缓存中
        // 如果已在缓存中，则更新值（保持缓存一致性）
        Object existingValue = localCache.getIfPresent(compositeKey);
        if (existingValue != null) {
            localCache.put(compositeKey, value);
            log.debug("更新本地缓存 {} = {}", compositeKey, value);
        }
    }
