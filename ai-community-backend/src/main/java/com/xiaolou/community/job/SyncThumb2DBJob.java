...
import com.xiaolou.community.model.enums.ThumbTypeEnum;
    ...
    public void syncThumb2DBByDate(String date) {  
        ...
        for (Object userIdBlogIdObj : allTempThumbMap.keySet()) {  
            ...
            if (thumbType == ThumbTypeEnum.INCR.getValue()) {  
                ...
                // 拼接查询条件，批量删除  
                needRemove = true;  
                wrapper.or().eq(Thumb::getUserId, userId).eq(Thumb::getPostId, blogId);
            } else {  
        ...
        if (!blogThumbCountMap.isEmpty()) {
            postMapper.batchUpdateThumbNum(blogThumbCountMap);
        }  
        // 异步删除  
