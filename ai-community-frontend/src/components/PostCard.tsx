  ...
  return (
    <Link to={`/post/${post.id}`} className="block group">
      <article className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-blue-100 transition-all duration-300 overflow-hidden">
        <div className={`flex gap-5 ${post.coverImg ? 'p-5' : 'p-6'}`}>
          {/* Cover Image - Left Side */}
          {post.coverImg && (
            <div className="w-44 h-32 shrink-0 overflow-hidden rounded-xl">
              <img
                src={post.coverImg}
                alt={post.title}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mb-3">
              {post.user?.userAvatar ? (
                ...
                <div className="w-7 h-7 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center ring-2 ring-blue-100">
            ...
            {/* Title */}
            <h2 className="text-base font-bold text-gray-900 mb-1.5 group-hover:text-blue-600 transition-colors line-clamp-2 leading-snug">
              {post.title}
            </h2>
